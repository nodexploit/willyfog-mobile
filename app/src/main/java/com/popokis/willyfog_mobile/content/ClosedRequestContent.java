...
import com.popokis.models.ClosedRequest;
...
public class ClosedRequestContent {
    ...
    public ClosedRequestContent(ProgressDialog dialog) {
        this.dialog = dialog;
        ...
        try {
            addAllItems(new ClosedRequestContent.GetUserRequests().execute("http://popokis.com:7000/api/v1/users/" + userId + "/requests", accessToken).get());
    ...
    private class GetUserRequests extends AsyncTask<String, String, List<UserRequests>> {
        ...
        protected List<UserRequests> doInBackground(String... data) {
            ...
            try {
                closedRequest = gson.fromJson((new SecureClient(accessToken)).get(url), ClosedRequest.class);
                result = closedRequest.getClosed();
            } catch (IOException e) {
                e.printStackTrace();
            }
